...
def namePublisher(name):

    #Idk what we're passing on the topic 
    message = name
    #display the message on the terminal
    rospy.loginfo("Published:" + message)
    #publish the message to the topic
    name_publisher.publish(message)
    rate.sleep()

def coordinatePublisher(result):
    #Idk what we're passing on the topic 
    coordinate = result['coords']
    #display the message on the terminal
    rospy.loginfo("Published:" + coordinate)
    #publish the message to the topic
    name_publisher.publish(coordinate)
    rate.sleep()


if __name__=='__main__':
    #Initialize Node for Publishing
    #define a topic to which the messages will be published

    #idk the message type for both of these, will have to change later
    name_publisher = rospy.Publisher('recognized_person', String, queue_size=10)

    coordinate_publisher = rospy.Publisher('recognized_person', type(result['coords']), queue_size=10)
    ...
    with dai.Device(create_pipeline()) as device:
        ...
        while True:
            ...
            try:
                # shouldnt we just focus on one person
                # + what happens if we encounter duplicate names
                namePublisher(names) 
                coordinate_publisher(result)
            except rospy.ROSInterruptException:
                pass
